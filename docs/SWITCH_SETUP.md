...
- [ ] microSD 卡：**16GB 即够用**（自制环境总占用 <500MB；本项目不走 emummc）。
      FAT32 格式（exFAT 在自制环境下问题多）。注意：NAND 完整备份约 30GB
      需要更大的卡——本机已决定放弃原系统，备份可跳过，以后想补再借大卡。
      **被 Switch 格式化后回电脑工作异常的卡**：多为文件系统损坏/假卡/主控报废，
      排查与修复流程见 §5 常见坑表尾。
...
| SD 卡被 Switch 格式化后回电脑不正常 | 三种可能：文件系统损坏（`wipefs -a` 后重建 FAT32 分区可修）/ 假卡写爆（`f3write`+`f3read` 验真实容量，远小于标称即假卡）/ 主控写保护报废（任何写操作都报 I/O 错误，换卡）。16G 新卡十几块，别在这上面耗时间 |
