...
版本仅来自构建元数据，不手写第二份版本号。首页使用初始化时缓存的低对比冷暖背景、
统一细分隔线、电脑图标和按键提示。视觉细节以本节和原生 renderer 为准，早期 HTML mock
仅保留信息架构参考。

设计参考：[Valve 新大屏介绍与官方截图](https://store.steampowered.com/news/posts/?enddate=1667322980)。
截图中的游戏封面、绿色状态和浅色焦点说明，大屏并非把所有控件染成蓝色。
本客户端采用中性石墨色表面；绿色用于游玩，琥珀色用于配对，紫色用于画面／设置图标，
珊瑚红用于移除及断开确认。颜色始终伴随文字或图标，不承担唯一的状态编码。
最近游戏只有真实标题和通用图形，不假设现有协议提供封面。

菜单、选项、设置、画面偏好和高级采用右侧 536px 面板，保留背后的首页或游戏；
72px 列表项使用统一图标盒与浅色选中面，不把每个操作都画成独立按钮卡片。
移除、断开、退出采用居中紧凑确认框，两按钮并排，默认焦点为取消。
确认框保留所属菜单作为背景，返回恢复原焦点。配对代码为四个琥珀色数字格；
帮助等短内容按内容收紧面板，返回按钮使用独立控件而非整条大色块。

弹层在 220ms 内位移并渐入，返回时在 160ms 内反向退场；侧栏以不透明表面从右边缘移入（536px 行程），
居中面板位移 20px 并渐隐，不采用弹跳。侧栏不做整面透明叠字，避免过渡中两页文字重影。位移与透明度由 UI model 统一计算；触摸命中
减去同一位移，不能只让 renderer 移动控件。退场完成前保留当前页面与本地输入所有权，
忽略重复激活；完成后才恢复父页面／远端输入，沿用按键释放屏障。
协议成功、断开等运行时事件可立即替换页面，不为装饰动画拖延协议状态。
普通控件保留 140ms 焦点描边过渡；侧栏使用明确的浅色选中项。

SDL renderer 拥有一个可复用的 1280×720 弹层 target texture，用于统一合成透明度；
初始化一次、退出时释放，不逐帧创建纹理。target 不可用时回退直接绘制，保留位移、
文字渐隐与相同命中坐标。背景与焦点资源同样由 renderer 持有；销毁顺序为
弹层／背景／字形纹理、字体、TTF，随后由媒体层释放 SDL renderer。
连接进度保留无百分比的轻量往返指示，不表示已完成的协议进度。串流仍无常驻装饰。
