...
设计定稿：2026-09-08。菜单清单、确认键位和视觉规则以本文件及 `app/src/ui/` 为准。
`ui-final.html` 仅保留早期流程参考，其中画框外的场景选择、模拟网络及模拟响应按钮
属于评审工具，不能进入正式应用。示例电脑、游戏和 Debug 数值不是实机数据。
本文件规定目标行为与代码边界，不承载实施状态。
实施任务、依赖和验收状态统一记录在 [Issue #9](https://github.com/kxn/nsteamlink/issues/9)。

## 1. 产品界面

- 启动即显示首页并后台发现电脑；没有欢迎向导、扫描完成确认、刷新按钮或自动连接。
- 首页顶部是电脑标签，多于一台时两侧显示 LB/RB 标志，L/R 肩键切换；每台电脑保留自己的焦点。发现新增设备只追加，不抢选择。
- 无任何记录及发现结果时，没有电脑占位卡。仅显示“正在查找电脑／尚未发现电脑”和
  “在同一网络中打开 Steam”；网络恢复后自动继续。短时间漏收广播不删除电脑。
- 新发现电脑显示广播名称；当前 IP 和本机配对状态直接显示在标签下方。标签色点为绿色（在线且已配对）、琥珀色（在线未配对）或灰色（暂未发现），并用短文字补充配对状态。
  不生成“客厅／书房”昵称，不凭广播推断账号、授权或游戏库。
- 已有配对和游玩记录的电脑显示最近游戏；记录属于该电脑和已确认账号。
  卡片固定 440×270 px，间距 24 px，图片区域 424×198 px 保留 Steam 横版比例，标题置于图片下方。
  标题固定单行、按可见字形垂直居中；未选中时末尾同一行省略。选中的长标题停留 1.2 秒后，
  以 30 px/s 往返滚动，两端各停 1.2 秒；换卡、换主机、关闭弹层或拖动列表后从开头重新计时。
  标题滚动严格裁切在文字区域内，不影响图片、卡片高度或点击区域。
  有更多记录时显示两张完整卡及下一张的一部分；少量记录不拉伸填满，不生成假卡。
  横向拖动可惯性吸附，滑动不触发启动；左右方向键／摇杆选择游戏，列表平滑跟随。
  没有历史时仅显示“打开 Steam”。新发现且无本机授权记录时仅显示“配对并连接”。
- 暂未发现的电脑保留上次名称，正文显示“暂未发现这台电脑”；隐藏启动入口。
  此状态不等于关机。标签下的地址标为“上次地址”。
- Y 直接打开 Steam（未配对时配对并连接）；不再设置电脑信息弹层。手动地址输入仅位于
  X 选项 → 设置 → 手动添加电脑；空状态、帮助正文不主动推荐输入地址。
...
- 从最近游戏直启时，将已加载封面从当前卡片图片区域放大为连接页背景：
  420 ms 四次方 ease-out，前段迅速推进、末段短促收稳，不弹跳；缩放保持原图比例，
  全屏采用 cover 居中裁切（裁掉超出屏幕的两侧／上下，不拉伸）。
  同步叠入预生成的虚化图和暗色遮罩，连接文字与取消按钮保持固定。
  首帧显示后背景在 180 ms 内淡出；无封面时使用原背景，启动不等待网络取图。
  每张封面上传时预生成小尺寸虚化纹理，动画热路径不做图片解码、回读或滤镜计算。
...
| 场景 | 手柄 | 触摸／鼠标 |
|---|---|---|
| 首页 | L/R 换电脑，左右只选择游戏，上下不切换区域，A 启动游戏（无历史时连接），B 退出确认，X 选项，Y 打开 Steam | 点标签换电脑；横向拖动卡片不启动，轻点游戏启动；各按钮执行自身动作 |
...
| 画面偏好 | 均衡/流畅/清晰对应协议 Balanced/Fast/Beautiful；独立带宽限制 4/6/10/20 Mbps，默认 6 Mbps。统一请求 H.264 720p60；真实协商值单独记录，不承诺 host 一定接受 |
...
输入测试必须检查发送给假 transport 的事件内容和顺序：点击 X 不产生流请求、Y 发起不带 game_id 的连接、触摸控件不泄露
...
游戏卡使用真实标题与实际下载的封面；未取得封面时显示通用进入图标。
...
珊瑚红用于移除入口的图标；确认框统一采用白色 A 操作。颜色始终伴随文字或图标，不承担唯一的状态编码。
最近游戏使用真实标题和 StoreBrowse/CDN 封面（协议来源见 STEAMLINK_PROTOCOL_RE §20），加载失败保留通用图形。卡片焦点采用浅亮底色与细描边，文字始终为浅色。

菜单、选项、设置和画面偏好采用右侧 536px 面板，保留背后的首页或游戏；
72px 列表项使用统一图标盒与浅色选中面，不把每个操作都画成独立按钮卡片。
移除、断开、退出采用居中紧凑确认框，两按钮并排。A 固定执行具名操作，B 固定取消，
按钮分别显示对应按键；A 操作使用唯一的白色强调，取消为中性底色。确认框不做方向焦点切换，
触摸按下哪个按钮就执行哪个按钮；错误框存在重试目标时为 A 重试／B 取消，否则只显示 B。
这替代先前“取消默认选中、确认有彩色强调”的设计：用户真机反馈证明该设计将 A 的焦点
语义与 B 的固定快捷键混用，视觉和操作不一致。
确认框保留所属菜单作为背景，返回恢复原焦点。配对代码为四个琥珀色数字格；
返回按钮使用独立控件而非整条大色块。
设置仅保留画面偏好、声音，以及非串流状态下的手动添加电脑。删除连接帮助、菜单快捷键
说明页和仅包一项／串流时为空的高级层级；开流时的一次性快捷键提示和用户手册继续保留。
声音控制即时本地静音；画面偏好和独立带宽限制在下次串流生效，修改一项不改变另一项。画面偏好采用原生绘制的单选圆点，
不使用字体勾号；实心圆点表示已保存值，白色行表示当前操作焦点，移动焦点不修改保存值。
...
游戏描边始终贴合正在滚动的卡片；普通控件保留 140ms 焦点描边过渡；侧栏使用明确的浅色选中项。
...
本地操作生效后播放短音：焦点移动/主机切换为轻点，确认进入为上扬双音，返回为低音，
设置改变为短切换音。A 与触摸使用同一动作入口，避免委托调用重复发声；边界、无效操作
和连续拖动保持安静。声音开关同时作用于串流与提示音；提示音与游戏声音混合而非打断。
提示音为程序生成的原创短音，无 Steam 音频资产。音频生命周期约束见 D-047。
